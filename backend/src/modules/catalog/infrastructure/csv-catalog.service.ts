...
export class CsvCatalogService implements OnModuleInit {
    ...
    private loadData() {
        ...
        this.lessons = leconsRaw.map(raw => {
            const l = new Lesson();
            l.id = raw.id;
            l.titre = raw.titre;
        ...
        // Note: CSV links exercises to modules; we attach each exercise to a lesson row (lessonId hierarchy).
        ...
        this.exercises = exercicesRaw.map(raw => {
            ...
            const lesson = this.lessons.find(l => l.moduleId === raw.moduleId);
            if (lesson) {
                e.lesson = lesson;
        ...
        console.log(`[CsvCatalogService] Data loaded: ${this.programmes.length} programmes, ${this.modules.length} modules, ${this.lessons.length} lessons, ${this.exercises.length} exercises.`);
    }
    ...
    getLessonsByModule(moduleId: string) { return this.lessons.filter(l => l.moduleId === moduleId); }
    getLessonById(id: string) { return this.lessons.find(l => l.id === id); }
    getExercisesByLesson(lessonId: string) { return this.exercises.filter(e => e.lessonId === lessonId); }
